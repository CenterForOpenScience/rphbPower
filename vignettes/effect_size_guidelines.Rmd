...
- **Multiple predictors**: The r_partial represents the unique effect of one predictor, so expect smaller values (0.05-0.25) than zero-order correlations
- **Model complexity**: Each additional predictor increases the required sample size
- **Logistic Regression Engine**: Note: The current validated engine (WebPower) performs calculations for a single predictor only. The n_predictors argument is ignored.
...
- **Validated and Fast Engines**: Replaces slow, error-prone simulations with fast, validated analytical formulas and delegation to trusted R packages
