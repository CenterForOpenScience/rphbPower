...
| Binary outcome prediction | Logistic Regression | `logistic_regression_power()` | Yes/no, success/failure; **Note: single predictor only** |
...
```{r log-regression}
# Note: The 'n_predictors' argument is ignored by the current engine.
# Power is calculated for a single predictor.
...
- **Engine Limitation**: The current validated engine (WebPower) is designed for a single predictor. The n_predictors argument is kept for compatibility but is ignored in the calculation
- Larger samples are often needed compared to linear regression for equivalent effects
- Use Cohen's d or r_partial as a reasonable effect size proxy for binary outcomes
...
```{r mixed-models}
# Calculate number of groups for a Level-1 (within-group) effect
mixed_models_power(
  r_partial = 0.20,
  power = 0.8,
  n_per_group = 20,
...
- **Test Level**: You must specify whether you are testing a level1 (within-group) or level2 (between-group) effect. The framework uses different validated engines for each (WebPower for L1, pwr for L2)
- **Intraclass Correlation (ICC)**: This parameter is critical and dramatically impacts power, especially for Level-1 effects
...
# Note: get total N by multiplying n_groups * n_per_group
mixed_result <- mixed_models_power(r_partial = research_effect, power = 0.8, n_per_group = 10, test_level = "level1")
mixed_n <- mixed_result$n_groups * mixed_result$n_per_group
