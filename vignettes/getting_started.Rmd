...
Here we'll plan a study with a binary outcome. The `logistic_regression_power` function now uses a robust simulation engine to provide accurate estimates.

```{r example_logistic, eval=FALSE}
# Research question: Does a new website design increase the probability of a user signing up?
# Expected effect from a previous A/B test is an odds ratio of 1.75.
# We also want to control for 2 other variables (e.g., user age, country).
pilot_or <- 1.75
n_covariates <- 2

# We can now input the odds ratio directly. The total number of predictors
# is our variable of interest (1) plus the covariates (2).
result <- logistic_regression_power(
  effect_input = pilot_or,
  effect_type = "or",      # Use the new "or" type
  power = 0.8,
  n_predictors = 1 + n_covariates # Total predictors = 3
)

# The function runs a simulation to find the required sample size.
result$n  # Required sample size: ~335
```
...
# Here, we calculate power for N=400 (20 groups of 20) to compare.
ml_result <- mixed_models_power(r_partial = training_effect, n_groups = 20, n_per_group = 20, test_level = "level1")
ml_power <- ml_result$power  # Power for a total N of 400

# Compare efficiency
comparison_table <- data.frame(
  Method = c("Correlation", "Regression (3 pred)", "Multilevel (N=400)"),
  Sample_Size = c(corr_n, reg_n, 400),
  Notes = c(paste0("N for 80% power: ", corr_n), 
            paste0("N for 80% power: ", reg_n),
            paste0("Power for N=400: ~", round(ml_power, 2)))
)

print(comparison_table)
# Corrected results: The multilevel model has much lower power (~0.45) for N=400
# compared to the other designs, highlighting the cost of clustering.
```
