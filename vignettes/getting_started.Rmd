...
# Note: The 'test_type' argument is obsolete; the function now uses a fast
# analytical engine based on the Aroian formula by default.
...
result$n  # Required sample size: ~105
result$indirect_effect  # Expected indirect effect size after discount: ~0.078
```
...
- **Validated and Fast Engines**: Replaces slow, error-prone simulations with fast, validated analytical formulas and delegation to trusted R packages (pwr, WebPower)
...
1. **Multiple methods**: Compare sample size requirements across approaches using unified metrics
2. **Model complexity**: Account for substantial increases with additional predictors
3. **Longitudinal designs**: Explore repeated measures, cross-lagged, or fixed effects approaches
4. **Mediation/SEM**: Use dedicated functions for indirect effects and structural models
