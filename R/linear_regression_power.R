...
#'
#' Performs a power analysis for a single coefficient in a multiple linear
#' regression model using the pwr package, which is based on Cohen's f-squared.
#'
#' @param r_partial The expected partial correlation for the predictor of interest. See the 'Planning for Multicollinearity' section for details.
#' @param n The total sample size.
#' @param power The desired statistical power (e.g., 0.8 for 80%).
#' @param n_predictors The total number of predictors in the final model.
#' @param alpha The significance level (Type I error rate, default = 0.05).
#' @param discount_factor The conservative planning discount factor (default = 0.75).
#' @param effect_input An alternative to `r_partial` for automatic conversion from other effect size metrics (e.g., d, f2).
#' @param effect_type The type of `effect_input`.
#' @return A list object containing the power analysis results.
#' @section Planning for Multicollinearity:
#' The `r_partial` argument is the cornerstone of this analysis. It represents the expected partial correlation between your predictor of interest and the outcome, *after controlling for all other predictors in the model*.
#'
#' It is critical **not** to use a simple zero-order correlation for this argument if your model contains multiple predictors. Doing so will result in a highly optimistic and likely inaccurate power analysis because it fails to account for the variance your predictor shares with the other covariates. A higher degree of multicollinearity will lead to a smaller partial correlation, which will require a larger sample size.
#' @export
